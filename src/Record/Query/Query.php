...
namespace Jungle\Data\Record\Query {
	...
	class Query{
		...
		public function handleCondition($left, $operator, $right){
			if(String::isCovered($left,'{','}')){
				$left = String::trimSides($left,'{','}');
			...
			if($operator){
				...
				if($right && is_string($right)){
					if(String::isCovered($right,'{','}')){
						$right = String::trimSides($right,'{','}');
		...
		public function pathInfo($path){
			...
			if(!isset($this->paths[$query_path])){
				...
				if($locator instanceof Record\Locator\Point){
					$locator = new Record\Locator\Path($origin_schema,$path,null,$extra,$locator);
				}
				$locator->extra = $extra;
