 ...
 * Time: 11:58
 */
namespace Ceive\DataRecord\Validation {
	
	use Ceive\DataRecord;
    use Ceive\DataRecord\Record;

    /**
	 * @see DataRecord\Operation - Похожая работа с Relation
	...
	class ValidationResult extends ValidationCollector{
		...
		/**
		 * абсолютная Отрисовка технического результата валидации
		 * Использование в целях дебагинга: <pre><?=$validation_result?></pre>
		 * @return string
		 */
		function __toString(){
			self::$_level++;
		...
		public function export(){
			...
			foreach($this->field_errors as $fieldKey => $validations){
				...
				foreach($validations as $validationRule){
					$fields[$fieldKey][] = $validationRule->type;
				}
			}
			$relations = [];
			foreach($this->related_error_validations as $relationKey => $results){
				$relations[$relationKey][] = $results->export();
			}
			return [
				'class'     => $this->record->getSchema()->getName(),
				'fields'    => $fields,
				'relations' => $relations
			];
		}
