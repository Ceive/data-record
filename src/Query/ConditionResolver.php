	...
	class ConditionResolver{
		...
		public function handleConditionArray(array $condition){
			if(isset($condition[self::TYPE_KEY])){
				...
				if(HelperArray::isAssoc($condition, true)){
					$a = [];
		...
		public function handleCondition($left, $operator, $right){
			...
			$this->right    = $right;
			if(HelperString::isCovered($this->left,$this->bracket_open,$this->bracket_close)){
				$this->left = HelperString::trimSides($this->left,$this->bracket_open,$this->bracket_close);
				$this->left = $this->handlePath($this->left);
			}
			if($this->operator){
				$this->operator = $this->handleOperator($this->operator);
				if($this->right && is_string($this->right)){
					if(HelperString::isCovered($this->right,$this->bracket_open,$this->bracket_close)){
						$this->right = HelperString::trimSides($this->right,$this->bracket_open,$this->bracket_close);
