 ...
 * Time: 23:51
 */
namespace Ceive\DataRecord\Condition {
	...
	class Condition{
		...
		public function handleCondition($left, $operator, $right){
			...
			if(($have = strcasecmp($operator,'have')===0) || ($nHave = strcasecmp($operator,'not have')===0)){
				...
				if(HelperString::isCovered($this->left,'{','}')){
					$this->left = HelperString::trimSides($this->left,'{','}');
					$extra = null;
				...
				if($this->operator){
					...
					if($this->right && is_string($this->right)){
						if(HelperString::isCovered($this->right,'{','}')){
							$this->right = HelperString::trimSides($this->right,'{','}');
					...
					if($i+1<$c){// если не множественная
						...
						foreach($relation->fields as $i => $field_name){
							...
							$condition[] = [
								...
								'=',
								HelperString::cover($schema->getOriginal($ref_field),'{','}')
							];
						}
