 ...
 * Time: 23:48
 */
namespace Ceive\DataRecord\Schema {

	use Ceive\DataRecord;

    use Ceive\DataRecord\Record;
	use Ceive\DataRecord\Collection;
	use Ceive\DataRecord\DataMap;
	...
	class Schema implements ValueAccessAwareInterface{
		 ...
		 * Если этот параметр равен лжи, то создаваемые объекты будут сразу же помечены состоянием @see \Ceive\DataRecord\Record::STATE_NEW
		...
		/**
		 * @param $key
		 * @return \Ceive\DataRecord\Field\Field|null
		 */
		...
		public function __construct($name){
			...
			$this->validations = [
				new \Ceive\DataRecord\Validation\CheckNullable(),
				new \Ceive\DataRecord\Validation\CheckField()
			];
		...
		/**
		 * @return array|\Ceive\DataRecord\Validation\Validation[]
		 */
		...
		/**
		 * @param Record $record
		 * @param \Ceive\DataRecord\Field\Field $pk_field
		 * @param StorageInterface $store
		 * @param $source
		 * @return float|int|null|string
		 */
		public function pkBeforeCreate(Record $record, \Ceive\DataRecord\Field\Field $pk_field, StorageInterface $store, $source){
			return null;
		}
		...
		public function initialize(Record $record = null){
			...
			// @PrimaryKey pk field (primary key)
			$this->pk = $this->pk?: key($this->fields) ;

			// @TitleKey tk field (title key)
			if(!$this->tk){
				foreach($this->fields as $name => $field){
			...
			foreach($this->relations as $name => $relation){
				if($relation instanceof RelationSchema && !$relation instanceof RelationForeignDynamic){
					if($relation instanceof RelationForeign){
						$referenced_schema = $relation->getSchemaGlobal($relation->referenced_schema);
						foreach($referenced_schema->relations as $n => $referenced){
							if($referenced instanceof \Ceive\DataRecord\Relation\RelationSchemaHost){
		...
		public function getForeignFromField($local_field){
			$relation_names = [];
			foreach($this->relations as $name => $relation){
				if($relation instanceof \Ceive\DataRecord\Relation\RelationForeign
...
//			}
		...
		public function getPoint($path){
			if(isset($this->locators[$path])){
				...
				if($relation instanceof \Ceive\DataRecord\Relation\RelationSchemaHost){
					$referenced_relation = $relation->getReferencedRelation();
					$point->to_many = $relation instanceof \Ceive\DataRecord\Relation\RelationMany;
					$point->in_host = true;
				...
				}elseif($relation instanceof \Ceive\DataRecord\Relation\RelationForeign && !$relation instanceof \Ceive\DataRecord\Relation\RelationForeignDynamic){
					$referenced_schema = $relation->getSchemaGlobal($relation->referenced_schema);
					...
					foreach($referenced_schema->relations as $name => $referenced){
						if($referenced instanceof \Ceive\DataRecord\Relation\RelationSchemaHost){
							if($referenced->getReferencedRelation() === $relation){
								...
								$point->reversed_to_many = $referenced instanceof \Ceive\DataRecord\Relation\RelationMany;
								$point->circular = $point->reversed_path === $point->path;
								$point->recursive = !$point->circular && $origin_schema === $point->schema;
								break;
							}
